...
const get_all_academic_faculties_from_db = async (query: Record<string, unknown>) => {

  const academic_faculty_query =new Query_Builder(Academic_Faculty_Model.find(), query).field_limit().filter().sort().pagination()
