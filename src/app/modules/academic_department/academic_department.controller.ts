...
const create_academic_department = catchAsync(async (req, res, next) => {
  const result =
    await academic_department_services.create_academic_department_to_db(
      req.body,
    );
  send_response(res, {
...
const get_single_academic_department = catchAsync(async (req, res, next) => {
  ...
  const result =
    await academic_department_services.get_single_academic_department_from_db(
      id,
    );
  send_response(res, {
...
const update_academic_department = catchAsync(async (req, res, next) => {
  const { id } = req.params;
  const result =
    await academic_department_services.update_academic_department_from_db(
      id,
      req.body,
    );
  send_response(res, {
