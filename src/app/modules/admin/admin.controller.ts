...
const get_single_admin = catchAsync(async (req, res, next) => {
  ...
  const result = await admin_services.get_single_admin_from_db(adminId);
  send_response(res, {
...
const delete_admin = catchAsync(async (req, res, next) => {
  const { adminId } = req.params;
  const result = await admin_services.delete_admin_from_db(adminId);
  send_response(res, {
...
const update_admin = catchAsync(async (req, res, next) => {
  ...
  const result = await admin_services.update_admin_from_db(adminId, admin_data);
